...
# code taken from https://github.com/aklein1995/intrinsic_motivation_techniques_study
class PPOAlgoICMAlain(PPOAlgo):
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.001, gae_lambda=0.95,
        ...
        if self.RGB=='False':
            input_size= 64 #size of the state embedding in case of grid encoding observations
            self.po_size= 7 #size of the image partial observability is 7
        else:
            input_size=40000 #size of the state embedding in case of RGB obs
            self.po_size=56 #size of the image partial observability is 56
        self.im_module = ICMModule(emb_network = EmbeddingNetwork_RIDE(),
                                           inv_network = InverseDynamicsNetwork_RIDE(num_actions=self.num_actions,input_size=input_size,device=self.device),
                                           forw_network = ForwardDynamicsNetwork_RIDE(num_actions=self.num_actions,input_size=input_size, device=self.device),
                                           device = device)
    ...
    def collect_experiences(self):
        for i in range(self.num_frames_per_proc):
            ...
            self.intrinsic_reward_per_frame=(torch.mean(self.intrinsic_reward_coeff*self.rewards_int[i])).item()
            self.log_episode_return += torch.tensor(reward, device=self.device, dtype=torch.float)
            self.log_episode_return_int += rewards_int_torch * self.intrinsic_reward_coeff
        ...
        shape_im = (self.num_frames_per_proc,self.num_procs, self.po_size,self.po_size,3) # preprocess batch observations (num_steps*num_instances, 7 x 7 x 3)
        ...
        input_obss = input_obss.view(self.num_frames_per_proc*self.num_procs,self.po_size,self.po_size,3)
        input_nobss = input_nobss.view(self.num_frames_per_proc*self.num_procs,self.po_size,self.po_size,3)
