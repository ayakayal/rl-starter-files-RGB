...
# class PPOAlgoPseudocount(PPOAlgo):
...
#             #print('obs after step',obs[0]['image'].shape)
...
#                 obs_tuple=obs[p]['image']
#                 #obs_tuple_tensor=torch.tensor(obs_tuple, device=self.device, dtype=torch.int32)
#                 # obs_tuple_tensor= self.preprocess_obss(obs_tuple, device=self.device)
#                 #print('obs tuple',obs_tuple_tensor)

#                 temp_irewards[p]=self.intrinsic_reward_coeff * self.pseudo_count_model.pc_reward(obs_tuple)

#                 # if tuple(obs_tuple) in self.train_state_count:
#                 #     self.train_state_count[tuple(obs_tuple)]+= 1
#                 # else:
#                 #     self.train_state_count[tuple(obs_tuple)]=1
#             #print('dict state count',self.train_state_count)
...
#             #self.intrinsic_rewards[i]=torch.tensor(temp_irewards).detach()
...
#             #print('self.intrinsic_rewards[i]',self.intrinsic_rewards[i])
#             #print('requires grad',self.intrinsic_rewards.requires_grad)
#             #print('self.intrinsic_rewards[i]',self.intrinsic_rewards[i])
...
import numpy as np
from torch_ac.algos import PPOAlgo
from VAE_density_model import vae_density
...
class PPOAlgoPseudocount(PPOAlgo):
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.001, gae_lambda=0.95,
        ...
        self.density_model= vae_density.VAEDensity(input_size=(56,56,3),
            num_skills=0,
            code_dim=128).to(self.device)

    def pass_models_parameters(self):
        return self.train_state_count

    def vae_pseudocount(self,obs_tuple):
        log_prob = self.density_model.get_output_for(obs_tuple, sample=False)
        self.density_model.update(obs_tuple)
        # self._n += 1
        stepped_log_prob = self.density_model.get_output_for(obs_tuple, sample=False)
        print(stepped_log_prob-log_prob)

        return stepped_log_prob-log_prob
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            for p in range(self.num_procs):
                ...
                obs_tuple_tensor=torch.tensor(obs_tuple, device=self.device, dtype=torch.float32).unsqueeze(0).view(1, -1)
                #uncomment for CTS density
                #obs_tuple_tensor=torch.tensor(obs_tuple, device=self.device, dtype=torch.int32)

                #print('obs tuple',obs_tuple_tensor.shape)
                #uncomment for VAE
                r=self.vae_pseudocount(obs_tuple_tensor)
                temp_irewards[p]=self.intrinsic_reward_coeff #* r
